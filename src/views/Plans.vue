    ...
    <div id="facade" class="plans-facade">
      <div ref="facadeInteractiveRef" class="plans-facade-interactive" @mouseleave="clearActiveZone">
        ...
        <article
          ...
          :class="{ 'is-available': activeVilla.status === 'available', 'is-sold': activeVilla.status === 'sold' }"
          :style="facadeCardStyle"
        >
          <div class="plans-facade-card-status">{{ t(`plans.statusValues.${activeVilla.status}`) }}</div>
          <div class="plans-facade-card-row">
            <strong>{{ t('plans.apartment') }}</strong>
            <span>{{ activeVilla.apartment }}</span>
          </div>
          <div class="plans-facade-card-row">
            <strong>{{ t('plans.rooms') }}</strong>
            <span>{{ activeVilla.pieces }}</span>
          </div>
          <div class="plans-facade-card-row">
            <strong>m2</strong>
            <span>{{ activeVilla.surface }} m<sup>2</sup></span>
          </div>
        </article>
      </div>
    </div>
    ...
    <div id="facade-table" class="plans-table">
      <div class="table plans-table-inner">
        <table class="avp-table avp-prix">
          ...
          <tbody>
            <tr v-for="v in villas" :key="v.lot" :class="['lot-' + v.lot, v.status === 'available' ? 'd' : 'r']">
              <td class="column-1">{{ v.apartment }}</td>
              <td class="column-3">{{ v.pieces }}</td>
              <td class="column-6">{{ v.surface }} m<sup>2</sup></td>
              <td class="column-7">{{ v.terrasse }}</td>
              <td class="column-8">{{ v.jardin }}</td>
              <td class="column-19">
                <a :href="fileUrl(v.planFileFr)" download class="plan-lang-link">FR</a>
              </td>
              <td class="column-20">
                <a :href="fileUrl(v.planFileDe)" download class="plan-lang-link">DE</a>
              </td>
              <td class="column-21"><i></i><span>{{ t(`plans.statusValues.${v.status}`) }}</span></td>
            </tr>
          </tbody>
        </table>
...
const allPlanFiles = computed(() => [...new Set(villas.flatMap((villa) => [villa.planFileFr, villa.planFileDe]))])
...
const facadeZones = [
  { id: 'top-left', overlay: 'imgs/plans/top-left.svg', villaIndex: 4, kind: 'path', d: 'M174.5 147.5H211.5V129H231V147.5H373V137.5H381V147.5H418.5V271.5H102L174.5 147.5Z' },
  { id: 'top-right', overlay: 'imgs/plans/top-right.svg', villaIndex: 5, kind: 'path', d: 'M418 147.562H447V137.526H455.5V147.562H594.499V130H614.223V147.562H651.141L727 272H418V147.562Z' },
  { id: 'mid-left', overlay: 'imgs/plans/mid-left.svg', villaIndex: 2, kind: 'rect', x: 133, y: 275, width: 285, height: 54 },
  { id: 'mid-right', overlay: 'imgs/plans/mid-right.svg', villaIndex: 3, kind: 'rect', x: 418, y: 275, width: 276, height: 54 },
  { id: 'bot-left', overlay: 'imgs/plans/bot-left.svg', villaIndex: 0, kind: 'rect', x: 133, y: 329, width: 285, height: 62 },
  { id: 'bot-right', overlay: 'imgs/plans/bot-right.svg', villaIndex: 1, kind: 'rect', x: 418, y: 329, width: 276, height: 62 },
]
...
const activeVilla = computed(() => (activeZone.value ? villas[activeZone.value.villaIndex] : null))
